...
def main():
	...
	try:
		opts, args = getopt.getopt(sys.argv[1:], "R:c:C:D:p:dhboms:r:U:P:t:x:T:", ["uri=","counter=", "process=", "level="])
...
def connector(config):
	...
	if config["retries"] < config["retry_count"]:
		if config["uri"] == "":
			...
			try:
				connection = MongoClient(config["uri"],serverSelectionTimeoutMS=2000, connectTimeoutMS=2000)
				retries = 0
				return connection
			except:
				retries += 1
